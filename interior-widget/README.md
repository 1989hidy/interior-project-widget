...
Notion의 공정관리 DB를 읽어 D-Day, 다음 공정, 진행률, 오늘의 공정을 보여 주는 작은 임베드 위젯입니다. 착공일은 별도 설정 DB 없이 **공정관리 DB의 시공 기간 중 가장 이른 시작일**로 자동 계산됩니다. Notion 비밀키는 브라우저에 노출되지 않고 Vercel의 서버 함수에서만 사용됩니다.
...
| `api/processes.js` | 공정관리 DB에서 공정 목록과 가장 이른 착공일을 읽는 안전한 서버 API |
...
4. Notion에서 `공정관리 DB`를 엽니다. 우측 상단 `···` → **Connections** → 만든 Integration을 찾아 연결합니다.
5. 공정관리 DB의 URL에서 긴 32자리 문자열을 복사합니다. `?v=` 앞의 값이 Database ID입니다.

이 프로젝트는 다음 속성 이름을 정확히 사용합니다.

- 공정관리 DB: `순서` (숫자), `공정명` (제목 또는 텍스트), `표시태그` (텍스트/선택), `상태` (상태/선택), `시공 기간` (날짜 범위)

착공일은 모든 공정의 `시공 기간` 시작일 중 가장 이른 날짜입니다. 따라서 보통 철거의 시작일이 자동으로 착공일이 됩니다. 더 이른 일정이 추가되면 그 일정의 시작일로 자동 변경됩니다.

속성 이름이 다르면 `api/processes.js` 안의 대괄호 속 이름을 Notion DB에 맞게 수정하세요.
...
`.env.local`을 열어 두 값에 실제 비밀값과 DB ID를 입력한 후 실행합니다.
...
3. **Environment Variables**에 `NOTION_TOKEN`, `NOTION_PROCESS_DATABASE_ID` 두 값을 추가합니다.
...
- 착공일은 `시공 기간`이 있는 공정 중 가장 이른 시작일입니다.
